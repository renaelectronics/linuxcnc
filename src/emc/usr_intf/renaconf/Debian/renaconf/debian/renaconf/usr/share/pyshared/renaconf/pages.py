...
# add large or common function calls to renaconf.py
...
#***********
    ...
    def start_finish(self):
        if self.w.importmach.get_active():
            ...
            from renaconf import import_mach
            self.d.load('/tmp/temp.renaconf', self)
            if not debug:
                os.remove('/tmp/temp.renaconf')
        elif not self.w.createconfig.get_active():
            filter = gtk.FileFilter()
            filter.add_pattern("*.renaconf")
            filter.set_name(_("LinuxCNC 'renaconf' configuration files"))
            ...
            if not self.d._lastconfigname == "" and self.d._chooselastconfig:
                dialog.set_filename(os.path.expanduser("~/linuxcnc/configs/%s.renaconf"% self.d._lastconfigname))
